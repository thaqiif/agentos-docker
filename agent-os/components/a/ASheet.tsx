...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50 bg-black/40 backdrop-blur-[3px] duration-300",
      className
    )}
    {...props}
    ref={ref}
...
const aSheetVariants = cva(
  "glass-thick glass-float fixed z-50 flex flex-col ease-[cubic-bezier(0.32,0.72,0,1)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-250 data-[state=open]:duration-[400ms]",
  {
    variants: {
      side: {
  ...
  (
    ...
    <ASheetPortal>
      ...
      >
        {children}
        {!hideCloseButton && (
          <SheetPrimitive.Close className="press focus-ring text-muted-foreground hover:text-foreground absolute top-4 right-4 flex size-7 items-center justify-center rounded-full hover:bg-[var(--fill-3)] disabled:pointer-events-none">
            <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("type-title-3 text-foreground", className)}
    {...props}
  />
));
