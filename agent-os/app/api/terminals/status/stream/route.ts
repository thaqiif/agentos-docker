...
/**
 * GET /api/terminals/status/stream
 *
 * Server-sent stream of terminal status snapshots, keyed by tmux session
 * name. Replaces the old 5-30s poll, which was why a terminal that started
 * working could take half a minute (or a manual refresh) to show as running.
 *
 * `?active=<tmuxName>` marks the terminal the user is looking at as seen,
